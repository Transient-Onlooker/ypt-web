...
- 일시정지는 열품타의 `stop`, 재개는 새 `start`입니다. 시작·재개 버튼을 누르면 서버가 앱 타이머가 정지 상태인지 다시 확인합니다. 일시정지 상태에서 종료할 때도 앱이 실제로 정지 상태인지 확인합니다. 앱이 꺼지거나 브라우저를 닫아도 자동 종료하지 않습니다. 화면 복귀 시와 열린 화면에서 15초 간격으로 상태를 다시 확인합니다.
...
- 기록 화면에서 이전·다음 날짜와 오늘로 이동할 수 있습니다. 공부 중인 시간은 완료된 오늘 기록과 구분해 표시하며, 다른 화면과 브라우저 탭 제목에서도 타이머를 볼 수 있습니다. 그룹에서는 공부 중 인원과 멤버별 상태·시간을 보여주며 닉네임 검색과 상태·시간·이름순 정렬을 제공합니다. 상태 갱신이 지연되거나 실패하면 마지막 확인 시각과 오류를 표시합니다.
