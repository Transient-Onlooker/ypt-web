...
- 일시정지는 열품타의 `stop`, 재개는 새 `start`입니다. 일시정지는 웹에 과목을 기억해 재개 버튼을 제공하고, 공부 끝내기는 웹 상태를 초기화해 다음 시작 때 과목을 고릅니다. 시작·재개 버튼을 누르면 서버가 앱 타이머가 정지 상태인지 다시 확인합니다. 일시정지 상태에서 종료할 때도 앱이 실제로 정지 상태인지 확인합니다. 앱이 꺼지거나 브라우저를 닫아도 자동 종료하지 않습니다. 화면 복귀 시와 열린 화면에서 15초 간격으로 상태를 다시 확인합니다.
...
- 기록은 API에서 확인된 날짜와 시간을 그대로 표시합니다. 과목별 시간이 응답에 없으면 추정값을 표시하지 않습니다. 가입 그룹 목록은 30초마다, 선택한 그룹의 멤버는 15초마다 갱신하고 직접 새로고침할 수 있습니다. 그룹의 `mc`는 정원으로 표시하고 현재 인원은 조회된 멤버 수로 표시합니다. 멤버의 현재 공부 상태는 날짜 로그의 `is` 값으로 표시하며, 필드가 없으면 미확인으로 표시합니다. 실행 중 시간은 완료 기록과 구분해 1초마다 증가시켜 보여줍니다.
