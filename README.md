...
- 기록은 API에서 확인된 날짜와 시간을 그대로 표시합니다. 과목별 시간이 응답에 없으면 추정값을 표시하지 않습니다. 가입 그룹은 그룹 화면에서 30초마다 갱신하고 직접 새로고침할 수 있습니다. 그룹의 `mc`는 정원으로 표시하고 현재 인원은 조회된 멤버 수로 표시합니다. 멤버의 현재 공부 상태는 API 필드 의미가 확인되지 않아 미확인으로 표시합니다.
